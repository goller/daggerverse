...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (any, error) {
	...
	case "MyFeature":
		...
		case "WithFlag1":
			...
			var flag string
			if inputArgs["flag"] != nil {
				err = json.Unmarshal([]byte(inputArgs["flag"]), &flag)
				if err != nil {
					fmt.Println(err.Error())
					os.Exit(2)
				}
			}
			return (*Flags).WithFlag1(&parent, flag), nil
		case "WithFlag2":
			var err error
			var parent Flags
			err = json.Unmarshal(parentJSON, &parent)
			...
			return (*Flags).WithFlag2(&parent, flag), nil
		case "Style1":
			var err error
			var parent Flags
			err = json.Unmarshal(parentJSON, &parent)
			...
			return (*Flags).Style1(&parent), nil
		case "Style2":
			var err error
			var parent Flags
			err = json.Unmarshal(parentJSON, &parent)
			...
			var flag1 Optional[string]
			if inputArgs["flag1"] != nil {
				err = json.Unmarshal([]byte(inputArgs["flag1"]), &flag1)
				if err != nil {
					fmt.Println(err.Error())
					os.Exit(2)
				}
			}
			var flag2 Optional[string]
			if inputArgs["flag2"] != nil {
				err = json.Unmarshal([]byte(inputArgs["flag2"]), &flag2)
				if err != nil {
					fmt.Println(err.Error())
					os.Exit(2)
				}
			}
			return (*Flags).Style2(&parent, flag1, flag2), nil
		default:
	...
	case "":
		return dag.CurrentModule().
			WithObject(
				dag.TypeDef().WithObject("MyFeature").
					WithFunction(
						dag.Function("WithFlag1",
							dag.TypeDef().WithObject("MyFeature")).
							WithArg("flag", dag.TypeDef().WithKind(Stringkind))).
					WithFunction(
						dag.Function("WithFlag2",
							dag.TypeDef().WithObject("MyFeature")).
							WithArg("flag", dag.TypeDef().WithKind(Stringkind))).
					WithFunction(
						dag.Function("Style1",
							dag.TypeDef().WithObject("Container")).
							WithDescription("example usage: \"dagger call with-flag --flag howdy style-1\"\n")).
					WithFunction(
						dag.Function("Style2",
							dag.TypeDef().WithObject("Container")).
							WithDescription("example usage: \"dagger call style-2 --flag howdy\"\n").
							WithArg("flag1", dag.TypeDef().WithKind(Stringkind).WithOptional(true)).
							WithArg("flag2", dag.TypeDef().WithKind(Stringkind).WithOptional(true))).
					WithField("Flag1", dag.TypeDef().WithKind(Stringkind)).
					WithField("Flag2", dag.TypeDef().WithKind(Stringkind))), nil
	default:
