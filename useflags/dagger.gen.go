...
type WithMyFeatureFunc func(r *MyFeature) *MyFeature

// With calls the provided function with current MyFeature.
//
// This is useful for reusability and readability by not breaking the calling chain.
func (r *MyFeature) With(f WithMyFeatureFunc) *MyFeature {
	return f(r)
}

func (r *MyFeature) Flag1(ctx context.Context) (string, error) {
	if r.flag1 != nil {
		return *r.flag1, nil
	}
	q := r.q.Select("flag1")
...
func (r *MyFeature) Flag2(ctx context.Context) (string, error) {
	if r.flag2 != nil {
		return *r.flag2, nil
	}
	q := r.q.Select("flag2")
...
func (r *MyFeature) Style1() *Container {
	q := r.q.Select("style1")
...
// MyFeatureStyle2Opts contains options for MyFeature.Style2
type MyFeatureStyle2Opts struct {
	Flag1 string
...
func (r *MyFeature) Style2(opts ...MyFeatureStyle2Opts) *Container {
	q := r.q.Select("style2")
	for i := len(opts) - 1; i >= 0; i-- {
		// `flag1` optional argument
		if !querybuilder.IsZeroValue(opts[i].Flag1) {
			q = q.Arg("flag1", opts[i].Flag1)
		}
		// `flag2` optional argument
		if !querybuilder.IsZeroValue(opts[i].Flag2) {
			q = q.Arg("flag2", opts[i].Flag2)
		}
	}
...
func (r *MyFeature) WithFlag1(flag string) *MyFeature {
	q := r.q.Select("withFlag1")
	q = q.Arg("flag", flag)
...
func (r *MyFeature) WithFlag2(flag string) *MyFeature {
	q := r.q.Select("withFlag2")
	q = q.Arg("flag", flag)
...
func (r *Client) MyFeature() *MyFeature {
	q := r.q.Select("myFeature")
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (any, error) {
	...
	case "Importer":
		...
		case "Style1":
			...
			return (*Importer).Style1(&parent), nil
		case "Style2":
			var err error
			var parent Importer
			err = json.Unmarshal(parentJSON, &parent)
			...
			return (*Importer).Style2(&parent), nil
		default:
	...
	case "":
		return dag.CurrentModule().
			WithObject(
				dag.TypeDef().WithObject("Importer").
					WithFunction(
						dag.Function("Style1",
							dag.TypeDef().WithObject("Container")).
							WithDescription("example usage: \"dagger call style-1\"\n")).
					WithFunction(
						dag.Function("Style2",
							dag.TypeDef().WithObject("Container")).
							WithDescription("example usage: \"dagger call style-2\"\n"))), nil
	default:
